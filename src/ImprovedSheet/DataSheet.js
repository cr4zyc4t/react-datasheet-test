...
export default class DataSheet extends PureComponent {
	constructor(props) {
		...
		const store = configureStore({
			preloadedState: this.defaultState,
			reducer: (state = this.defaultState, { type, payload }) => {
				switch (type) {
					case "SET":
	...
	_setState(state) {
		if (this.isSelectionControlled() && ("start" in state || "end" in state)) {
			let { ...rest } = state;
			let { start, end } = this.store.getState();
	...
	handleCut(e) {
		if (isEmpty(this.store.getState().editing)) {
			e.preventDefault();
			this.handleCopy(e);
	...
	onMouseDown(i, j, e) {
		const isNowEditingSameCell =
			!isEmpty(this.store.getState().editing) &&
			this.store.getState().editing.i === i &&
			this.store.getState().editing.j === j;
		let editing =
			isEmpty(this.store.getState().editing) ||
			this.store.getState().editing.i !== i ||
			this.store.getState().editing.j !== j
				? {}
				: this.store.getState().editing;

		this._setState({
			selecting: !isNowEditingSameCell,
			start: e.shiftKey ? this.store.getState().start : { i, j },
			end: { i, j },
			editing: editing,
	...
	onMouseOver(i, j) {
		if (this.store.getState().selecting && isEmpty(this.store.getState().editing)) {
	...
	isEditing(i, j) {
		return this.store.getState().editing.i === i && this.store.getState().editing.j === j;
	}

	isClearing(i, j) {
		return this.store.getState().clear.i === i && this.store.getState().clear.j === j;
	}
	...
	render() {
		...
		const { forceEdit } = this.store.getState();
			...
			<Provider store={this.store}>
				<span
					...
					<MySheet
						...
						// editValue={this.store.getState().editValue}
